...
#
#' @title RIntegrationPack Package
#'
#' @description
...
#' RIntegrationPack provides the metric expression of each potential output.  
...
#'
#' @references RIntegrationPack. \url{http://RIntegrationPack.codeplex.com/}.
#' @docType package
#' @name RIntegrationPack
#' @aliases RIntegrationPack
NULL    #Required to avoid warnings when publishing documentation

###-------------------------------------------------------------------------+
### Pop-up a message when the package is attached
.onAttach <- function(libname, pkgname) {
  if ("RIntegrationPack" %in% getOption("defaultPackages"))
    deployR()
  else {
    msg <- paste("RIntegrationPack Package\n",
                 "Copyright (c) 2013 MicroStrategy, Inc.\n",
...
#'
#' @description
#' \code{deployR} launches the MicroStrategy R Analytic Deployer utility
...
#' an R Script, capture its "signature" (the nature of inputs and outputs 
#' along with other information about how the R analytic should be executed).
#' 
#'
#' @references RIntegrationPack. \url{http://MicroStrategyRIP.codeplex.com/}.
#' @name deployR
#' @aliases deployR
...
deployR <- function() {
  ...
  MSTR_WORKINGDIR <- "mstrWorkingDir"
  ...
  TrackParams <- function(pName="", isUsed=TRUE) {
    if(DEBUG<-TRUE) print(paste0("Function enter: TrackParams=", pName, "<-", isUsed))
    if(nchar(pName)>0) {
  ...
  GetParam <- function(x) {
    if(DEBUG<-TRUE) print(paste0("Function enter: GetParam=", x))
  ...
  SetFunctionID <- function(thisOutput=NA) {    
    if(DEBUG<-TRUE) print("Function enter: SetFunctionID")
  ...
  SetOutputs <- function(...) {
    ...
    if(DEBUG<-TRUE) print("Function enter: SetOutputs")    
  ...
  GetParamValue <- function(metExp, pName) {
    if(DEBUG<-TRUE) print(paste0("Function enter: GetParamValue, MetExp=", metExp,", pName=", pName))
  ...
  UpdateExpression <- function(thisOutput=NA) {
    if(!envRAD$allowUpdates) return()    #Don't process actions while updates are NOT allowed
    if(DEBUG<-TRUE) print(paste0("Function enter: UpdateExpression=", thisOutput))    
  ...
  GetCommentBlock <- function() {
    if(DEBUG<-TRUE) print("Function enter: GetCommentBlock")
    ...
    MSTR_STATEMENT    <- paste0(MSTR_EXFLAG, " <- (exists(\"", MSTR_EXFLAGIN, "\"))")
    MSTR_COMMENT      <- "#Flag if executed by MicroStrategy"
    commentBlock <- paste0(commentBlock, "#\n", MSTR_STATEMENT, 
                           ifelse(svalue(envRAD$chkDir), "               ", "  "), 
                           MSTR_COMMENT, "\n")
  ...
  SaveToFile <- function(...) {
    if(DEBUG<-TRUE) print("Function enter: SaveToFile")
  ...
  OkToSave <- function() {
    if(DEBUG<-TRUE) print("Function enter: OkToSave")
...
#      }
  ...
  ScriptSave <- function(...) {
    if(DEBUG<-TRUE) print("Function enter: ScriptSave")
  ...
  ScriptOpen <- function(...) {
    if(DEBUG<-TRUE) print("Function enter: ScriptOpen")
    ...
    sErrMsg <- tryCatch({  
      envRAD$g.script <- scan(envRAD$g.file, 
                              what=character(), 
                              sep="\n",  
                              quiet=!(DEBUG<-TRUE))        #Persist the script for use when saving
    ...
    if(nchar(sErrMsg)>0) {                                 #There was an error
    ...
    tokens <- attr(scriptParsed, "data")                 #The script tokens are in this data.frame
     if(DEBUG<-TRUE) TOKENS <<- tokens                   #For debugging, make a global copy of the TOKENS
    ...
    if(length(mstrVarStart)>0) {  
      ...
      mstrVars <- mstrVars[(tokens[mstrVars, "token.desc"]=="COMMENT")]                                            #Get all the Comment lines
    ...
    symbols <- symbols[!(tokens[symbols, "text"] %in% EXCLUDED_NAMES)]                      #Exclude certain variables that used only by MicroStrategy, never as inputs, outputs or parameters
    ...
    if(priorVarsExist) {
      dir <- append(dir, rep("Unused", length(symbols)))
    } else {
  ...
  CreateDialog2 <- function() {
    if(DEBUG<-TRUE) print(paste0("Function enter: CreateDialog2"))
    ...
    UpdateVars <- function(varCtrl) {
      if(DEBUG<-TRUE) print(paste0("Function enter: UpdateVars, varName=", id(varCtrl)))
    ...
    LabelInputs <- function(rptCt) {
      if(DEBUG<-TRUE) print("Function enter: LabelInputs")      
    ...
    LabelOutputs <- function() {
      if(DEBUG<-TRUE) print("Function enter: LabelOutputs")      
    ...
    ColorButton <- function(btn, dir) {
      if(DEBUG<-TRUE) print(paste0("Function enter: ColorButton, var=", svalue(btn), ", dir=", dir))
      spec <- switch(dir, 
    ...
    SetParamValue <- function(edtPvalue, param) {
      if(DEBUG<-TRUE) print(paste0("Function enter: SetParamValue, param=", param, ", pvalue=", svalue(edtPvalue)))
    ...
    SetParamIndex <- function(param, pList) {
      if(DEBUG<-TRUE) print(paste0("Function enter: SetParamIndex, param=", param))
    ...
    SetParamCtrl <- function(dlParam, newParam, oldParam=NULL) {
      if(DEBUG<-TRUE) print(paste0("Function enter: SetParamCtrl, newParam=", newParam, ", oldParam=", oldParam))
    ...
    UpdateParamLists <- function() {
      if(DEBUG<-TRUE) print("Function enter: UpdateParamLists")      
    ...
    SetMaxRptCt <- function() {
      if(DEBUG<-TRUE) print("Function enter: SetMaxRptCt")
    ...
    UpdateControl <- function(varCtrl) {
      if(DEBUG<-TRUE) print(paste0("Function enter: UpdateControl, varCtrl=", id(varCtrl)))
      envRAD$allowUpdates <- FALSE
      dir <- tag(varCtrl, "dir")
    ...
    MoveVar<- function(varCtrl, from, to) {
      if(DEBUG<-TRUE) print(paste0("Function enter: MoveVar, varCtrl=", id(varCtrl), " from=", from, " to=", to))
    ...
    ParamValueHandler <- function(h, ...) {
      if(envRAD$allowUpdates) {         #Don't process actions while updates are NOT allowed
        if(DEBUG<-TRUE) print(paste0("Handler enter: edtPvalue, param=", svalue(h$obj)))
    ...
    AddParamValueHandler <- function(edtPvalue) {
      if(DEBUG<-TRUE) print(paste0("Function enter: AddParamValueHandler, varName=", id(tag(edtPvalue, "varCtrl"))))
      ...
      if(DEBUG<-TRUE) print(paste0("Function exit: AddParamValueHandler, varName=", id(tag(edtPvalue, "varCtrl")), 
    ...
    ParamChangedHandler <- function(h, ...) {  #Add handler after selection is made
      if(DEBUG<-TRUE) print(paste0("HANDLER_NEW: dlParam-Changed=", id(tag(h$obj,"varCtrl"))))
      if(envRAD$allowUpdates) {   #Don't process actions while updates are NOT allowed
        if(DEBUG<-TRUE) print(paste0("--> HANDLER_ENTER: dlParam-Changed=", id(tag(h$obj,"varCtrl"))))
        envRAD$allowUpdates <- FALSE
        ...
        UpdateVars(tag(h$obj, "varCtrl"))
        UpdateParamLists()                                     #Update the Parameter Lists that are in use
        if(DEBUG<-TRUE) print(paste0("--> HANDLER_EXIT: dlParam-Changed=", id(tag(h$obj,"varCtrl"))))
      }
    }  ##END-ParamChangedHandler
    
    AddParamChangedHandler <- function(dlParam) {
      if(DEBUG<-TRUE) print(paste0("Function enter: AddParamChangedHandler, varName=", id(tag(dlParam, "varCtrl"))))
      ...
      if(DEBUG<-TRUE) print(paste0("Function exit: AddParamChangedHandler, varName=", id(tag(dlParam, "varCtrl")), 
    ...
    CreateVarCtrl <- function(varName, dir, dType, pType, param, pvalue) {
      if(DEBUG<-TRUE) print(paste0("Function enter: CreateVarCtrl, varName=", varName))
      UpdateStatusBar()
      ...
      edtPvalue <- gedit(text=tag(varCtrl, "pvalue"), initial.msg=PARAM_DEFAULT, container=gPar, editable=TRUE, expand=TRUE, width=10, font.attr=c(size=8),
                         handler=function(h,...) {
                           if(envRAD$allowUpdates) {         #Don't process actions while updates are NOT allowed
                             if(DEBUG<-TRUE) print(paste0("edtPvalue Handler: ", svalue(dlParam), "=", svalue(h$obj)))
                             envRAD$allowUpdates <- FALSE
    ...
    envRAD$chkScr <- gcheckbox("Use Default R Scripts Folder", checked=FALSE, container=fScript,  
...
#    glabel("Count", container=gTopRight, editable=FALSE, anchor=c(-1,1))              #2nd word label for Repeat Count
  ...
  evalq({
    ...
    if(DEBUG<-TRUE) print("RAD: Member variables Created")
...
if(DEBUG<-TRUE) deployR()   #Auto-launch when in DEBUG mode