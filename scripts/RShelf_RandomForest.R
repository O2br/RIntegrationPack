...
#RVAR NumTree -parameter NumericParam1
#RVAR NumVar -parameter NumericParam2
...
#EXP-v1 Class: RScript<_RScriptFile="RandomForest.R", _InputNames="Target, Vars", BooleanParam9=TRUE, StringParam9="RandomForest", NumericParam1=750, NumericParam2=3, NumericParam3=42>(Target, Vars)
#EXP-v1 ClassId: RScript<_RScriptFile="RandomForest.R", _InputNames="Target, Vars", _OutputVar="ClassId", BooleanParam9=TRUE, StringParam9="RandomForest", NumericParam1=750, NumericParam2=3, NumericParam3=42>(Target, Vars)
#
#Version 2.0 Metric Expressions (uses _Params for parameters):
#EXP-v2 Class: RScript<_RScriptFile="RandomForest.R", _InputNames="Target, Vars", _Params="TrainMode=TRUE, FileName='RandomForest', NumTree=750, NumVar=3, Seed=42">(Target, Vars)
#EXP-v2 ClassId: RScript<_RScriptFile="RandomForest.R", _InputNames="Target, Vars", _OutputVar="ClassId", _Params="TrainMode=TRUE, FileName='RandomForest', NumTree=750, NumVar=3, Seed=42">(Target, Vars)
#
#MICROSTRATEGY_END
...
mstr.ErrMsg <- tryCatch({                                      #tryCatch for Exception Handling
  ...
  #Load packages
  CheckInstallPackages("randomForest")                         #Load the "randomForest" package
  ...
  if(exists("mstr.ExFlag")) {                                  #If this is executed by MicroStrategy
    ...
    Target <- ifelse(runif(1000)>.5,1,0)                       #  Set the factors of the target variable
    df <- data.frame(cbind(Target, var1, var2, var3))          #  Create a data frame from the input variables
    NumTree <- 500                                             #  Set modeling parameter
    NumVar <- 3                                                #  Set modeling parameter
    TrainMode=TRUE                                             #  Assuming we're training
    FileName <- "RandomForest_Console"
  }
  
  #Modeling
  if(TrainMode) {                                              #If training,
    model <- randomForest(x=df[, -1],
                                  y=factor(df[, 1]),
                                  ntree=NumTree,
                                  mtry=NumVar)                 #  Create the model
  ...
  Class <- as.character(predict(model,
                           newdata = df[, -1]))                #Return predictions from the model
