...
#RVAR Correction -parameter NumericParam1
#
#RVAR Class -output -string -vector#RVAR ClassId -output -numeric -vector
#
#Version 1.0 Metric Expressions (uses the 27 Pre-defined Parameters):
#EXP-v1 Class: RScript<_RScriptFile="NaiveBayes.R", _InputNames="Target, Vars", BooleanParam9=TRUE, StringParam9="NaiveBayes", NumericParam1=1>(Target, Vars)
#EXP-v1 ClassId: RScript<_RScriptFile="NaiveBayes.R", _InputNames="Target, Vars", _OutputVar="ClassId", BooleanParam9=TRUE, StringParam9="NaiveBayes", NumericParam1=1>(Target, Vars)
#
#Version 2.0 Metric Expressions (uses _Params for parameters):
#EXP-v2 Class: RScript<_RScriptFile="NaiveBayes.R", _InputNames="Target, Vars", _Params="TrainMode=TRUE, FileName='NaiveBayes', Correction=1">(Target, Vars)
#EXP-v2 ClassId: RScript<_RScriptFile="NaiveBayes.R", _InputNames="Target, Vars", _OutputVar="ClassId", _Params="TrainMode=TRUE, FileName='NaiveBayes', Correction=1">(Target, Vars)
#
#MICROSTRATEGY_END
...
mstr.ErrMsg <- tryCatch({                                      #tryCatch for Exception Handling
  ...
  #Load packages
  CheckInstallPackages("e1071")                                #Load the "e1071" package
  ...
  if(exists("mstr.ExFlag")) {                                  #If this is executed by MicroStrategy
    ...
    FileName <- "NaiveBayes_Console"                           #  Set model name for output files
  }
  
  #Modeling
  if(TrainMode) {                                              #If training,
    model <- naiveBayes(factor(df[, 1]) ~ .,
                                data=df[, -1],
                                laplace=Correction)            #  Create the model
