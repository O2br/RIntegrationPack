...
#EXP-v1 Probability: RScript<_RScriptFile="StepwiseLogistic.r", _InputNames="Target, Vars", BooleanParam1=TRUE>(Target, Vars)
#
#Version 2.0 Metric Expression (uses _Params for parameters):
#EXP-v2 Probability: RScript<_RScriptFile="StepwiseLogistic.r", _InputNames="Target, Vars", _Params="Stepwise=TRUE">(Target, Vars)
#
#MICROSTRATEGY_END
...
mstr.ErrMsg <- tryCatch({                                      #tryCatch for Exception Handling
  ...
  #Finish
  try(print("Success!"))                                       #Print completion message when run from the console                                             mstr.ErrMsg <- ""                                            #If we made it here, no errors were caught (using try to continue on any print error)
